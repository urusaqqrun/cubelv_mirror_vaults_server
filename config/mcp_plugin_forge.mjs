...
const TOOL_DEF = {
  name: 'plugin_forge',
  description: '啟動插件鍛造 Sub-Agent 建立新插件。只需呼叫一次，Sub-Agent 會自動處理前端 UI 和後端服務（如果需要）。不要分開呼叫前端和後端，一次呼叫即完成所有工作。',
  inputSchema: {
    type: 'object',
    properties: {
      title: { type: 'string', description: '插件名稱（如「番茄鐘插件」）' },
      prompt: { type: 'string', description: '用戶的完整需求描述，包含所有前端和後端需求' },
    },
...
async function forgePlugin(args) {
  ...
  const postData = JSON.stringify({ title, prompt, memberID, wsSessionID, forgeType: 'ui' });
